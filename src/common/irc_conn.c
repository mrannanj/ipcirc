...
size_t irc_conn_pack_row(uint8_t * buf, struct irc_row * r)
{
	...
	amessage.row = &row;
	len = amessage__get_packed_size(&amessage);
	nlen = htons(len);
...
{
	struct irc_data *irc = c->data.ptr;
	int j, k = IRC_NLINES + irc->cpos - irc->cn;
	for (j = 0; j < irc->cn; ++j, k = (k + 1) % IRC_NLINES) {
		uint8_t buf[2048];
...
void irc_send(struct epoll_cont *e, struct conn *c, const char *message, ...)
{
	...
	n = vsnprintf(buf, IRC_MAXLEN - 3, message, vl);
	if (n < 0) {
		log("vsnprintf error");
		return;
	}
